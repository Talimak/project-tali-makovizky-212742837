...
if (form) {
  form.addEventListener('submit', function(e) {
    ...
    const isClosed = document.getElementById('isClosed').value;
    const userType = document.getElementById('userType').value;
    const email = document.getElementById('email').value.trim();

    if (boxId === '' || isNaN(boxId) || boxId < 1 || boxId > 999) {
      errors.push('מזהה תיבה חייב להיות בין - 1 ל - 999');
    }
    if (boxColor === '') {
      errors.push('יש להזין צבע תיבה');
    }
    ...
    if (isClosed === '') {
      errors.push('יש לבחור האם התיבה נסגרה');
    }
    if (tone === 'רגזן' && Number(mailCount) === 0) {
      errors.push('תיבה עם טון רגזני אינה יכולה להכיל רק 0 מכתבים');
    }
